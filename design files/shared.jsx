// Shared icons, Nav, Footer, common components
// Exposes globals via window.* at end of file
...
const Icon = {
  ...
  facebook: () =>
  <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13 22V12h3l1-4h-4V6c0-1 .5-2 2-2h2V0h-3c-3 0-5 2-5 5v3H6v4h3v10z" /></svg>,
  ...
  twitter: () =>
  <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18 3h3l-7 8 8 10h-6l-5-6-5 6H3l7-9L3 3h6l4 5z" /></svg>,

  youtube: () =>
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="2" y="5" width="20" height="14" rx="3" /><path d="M10 9 L16 12 L10 15 Z" fill="currentColor" /></svg>,
    ...
    <nav className={`navbar ${scrolled ? "scrolled" : ""}`}>
      <div className="nav-inner">
        <div className="brand" onClick={() => navigate("home")}>
          <div className="brand-crest"><Icon.crest /></div>
        ...
        <ul className="nav-links">
          ...
          <li>
            <a className="nav-cta" href="#admissions" onClick={(e) => {e.preventDefault();navigate("admissions");}} style={{ padding: "6px 12px" }}>
              Apply Now
            </a>
          </li>
        </ul>
        <button className="nav-burger" onClick={() => setMobileOpen((o) => !o)} aria-label="menu">
          <svg viewBox="0 0 24 24" width="22" height="22" stroke="currentColor" strokeWidth="2" fill="none"><path d="M4 7h16M4 12h16M4 17h16" /></svg>
        </button>
      </div>
      <div className={`nav-mobile ${mobileOpen ? "open" : ""}`}>
        {PAGES.map((p) =>
        <a key={p.id} href={`#${p.id}`} onClick={(e) => {e.preventDefault();navigate(p.id);setMobileOpen(false);}}>{p.label}</a>
        )}
        <a className="nav-cta" href="#admissions" onClick={(e) => {e.preventDefault();navigate("admissions");setMobileOpen(false);}}>Apply Now</a>
      </div>
    </nav>);
    ...
    <footer className="footer">
      <div className="footer-grid">
        <div>
          ...
          <p>A world-class nursery through secondary education in the heart of Enugu State — where every child is raised to soar.</p>
        ...
        <div>
          ...
          <ul>
            {PAGES.slice(1).map((p) =>
            <li key={p.id}>
                <a href={`#${p.id}`} onClick={(e) => {e.preventDefault();navigate(p.id);}}>{p.label}</a>
              </li>
            )}
          </ul>
        </div>
        <div>
          ...
          <div className="footer-contact-row">
            <Icon.pin />
            <div>Independence Layout,<br />Enugu, Enugu State, Nigeria</div>
          </div>
          <div className="footer-contact-row">
            <Icon.phone />
...
// ============== Page Hero Banner ==============
function PageHero({ title, subtitle, breadcrumb, navigate, children }) {
  return (
...
function CountUp({ value, duration = 1800 }) {
  // Parse a leading number out of strings like "500+", "98%", "Top 5"
...
// expose globals
Object.assign(window, { Icon, Navbar, Footer, PageHero, StatsStrip, CountUp, PAGES });